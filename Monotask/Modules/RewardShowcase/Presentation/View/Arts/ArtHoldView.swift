...
struct ArtHoldView: View {
    ...
    var svgView: some View {
        let view = SVGView(contentsOf: Bundle.main.url(forResource: "artHoldFlower", withExtension: "svg")!)
        
        if let personVector = view.getNode(byId: "personHoldPotVector") {
    ...
    func startSmoothOscillation(for part: SVGNode) {
        ...
        personAnimationTimer = Timer.scheduledTimer(withTimeInterval: 0.02, repeats: true) { _ in
            self.yOffset += self.yDirection * 0.02
            
            if self.yOffset >= 1.5 {
                self.yDirection = -1
            } else if self.yOffset <= -1.5 {
                self.yDirection = 1
            }
            
            withAnimation(Animation.linear(duration: 0.01)) {
                part.transform = CGAffineTransform(translationX: self.yOffset, y: 0)
            }
        }
    }
